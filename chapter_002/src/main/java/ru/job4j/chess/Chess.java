...
public class Chess extends Application {
    ...
    private Rectangle buildFigure(int x, int y, int size, String image) {
        ...
        rect.setOnMouseReleased(event -> {
            try {
                this.board.move(this.findBy(momento.getX(), momento.getY()), this.findBy(event.getX(), event.getY()));
    ...
    private Group buildGrid() {
        ...
        for (int y = 0; y != this.size; y++) {
            for (int x = 0; x != this.size; x++) {
                panel.getChildren().add(this.buildRectangle(x, y, 40, (x + y) % 2 == 0));
            }
        }
        return panel;
    }
    ...
    public void add(Figure figure, Group grid) {
        ...
        grid.getChildren().add(this.buildFigure(position.x * 40 + 5, position.y * 40 + 5, 30, figure.icon()));
    }
